document.addEventListener('DOMContentLoaded', function () {
  ...
  sections.slice(1).forEach(sec => {
    ...
    const maxRows = Math.max(sec.instructions.length, sec.reps.length);

    let rows = '';
    for (let i = 0; i < maxRows; i++) {
      rows += `
        <tr>
          <td>${sec.instructions[i] ? sec.instructions[i] : ''}</td>
          <td>${sec.reps[i] ? sec.reps[i] : ''}</td>
        </tr>
      `;
    }
